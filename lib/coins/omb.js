...
function Coin(data){
    ...
    this.getBlockHeaderByHash = function(blockHash, callback){
        global.support.rpcDaemon('getblock', {"hash": blockHash}, function (body) {
            if (typeof(body) !== 'undefined' && body.hasOwnProperty('result')){
              var blockJson = JSON.parse(result.body.json);
              body.result.block_header.reward = 0;

              var minerTx = blockJson.miner_tx;
              if (minerTx.vout.length != 2) {
                 console.error('Coinbase transaction vout size is not 2');
              }
